...
public:
  ...
  JPetMCHit(UInt_t MCDecayTreeIndex, UInt_t MCVtxIndex, float Energy, float Time,
	    const TVector3& Position, const TVector3& Polarization, const TVector3& Momentum,const TVector3& MomentumOut);
  UInt_t getMCDecayTreeIndex() const;
  UInt_t getMCVtxIndex() const;
  const TVector3& getPolarization() const;
  const TVector3& getMomentum() const;
  const TVector3& getMomentumOut() const;
  ...
  void setMomentumOut(Float_t gg, Float_t hh, Float_t kk);
